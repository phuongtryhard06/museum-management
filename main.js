 ...
 */
/**
 * Điều hướng Menu trên thiết bị di động (Mobile Hamburger Toggle)
 */
function toggleMobileNav() {
  const strip = document.getElementById('headerNavStrip');
  const toggleBtn = document.getElementById('mobileNavToggle');
  ...
  const isOpen = strip.classList.toggle('mobile-open');
  if (toggleBtn) {
...
/**
 * Alias cho toggleMobileNav để tránh lỗi giao diện
 */
